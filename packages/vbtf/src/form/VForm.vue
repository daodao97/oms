...
export default defineComponent({
  ...
  props: {
    ...
    // 表单初始值的来源 1, modelValue 2, formItems.*.value 3 query
    // query 优先级最高
    // modelValue 和 defaultValue
    // 默认 valueSourcePriority = 0 时,  modelValue > defaultValue
    // valueSourcePriority = 1 时,modelValue < defaultValue
    // 已满足不同场景默认值问题
    valueSourcePriority: {
      type: Number,
      default: 0
  ...
  methods: {
    ...
    init(formItems) {
      // 表单字段值的来源及优先级
      // 默认 defaultValue < modelValue < query
      const formData = this.$props.valueSourcePriority === 0 ? merge(this.defaultValue(formItems), this.$props.modelValue) : merge(this.$props.modelValue, this.defaultValue(formItems))
      const fieldMap = {}
      const computeRules = {}
