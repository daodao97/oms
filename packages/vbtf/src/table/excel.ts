...
const generateArray = (table: HTMLElement, max : number) => {
  ...
  const ranges = []
  for (let R = 0; R < (max || rows.length); ++R) {
    const outRow = []
    const row = rows[R]
    if (!row) {
      continue
...
export const exportTable2Excel = (id: string, max: number, name: string) => {
  const theTable = document.getElementById(id)
  if (theTable) {
    const oo = generateArray(theTable, max)
    const ranges = oo[1]
    ...
    }), (name || (new Date()).toTimeString()) + '.xlsx')
  }
}
